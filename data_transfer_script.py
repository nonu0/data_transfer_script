...
database = 'db_name'  # Replace with the path or name of your SQLite database file
table = 'Wanaanga.dbo.Customer'  # the name of your table in SQLite
balances_table = 'balances'  # the name of the balances table in SQLite
server = 'localhost,1433'
username = 'sa'
password = 'yourStrong(!)Password'  # Replace with the same password you used when starting the container
column_id = 'acno'  # the id column of your table in SQLite
...
# Function to fetch member data from the MSSQL database in batches
...
def fetch_member_data(start_index, batch_size):
    try:
        ...
        for row in rows:
            ...
            if len(acno_parts) >= 2:
                middle_portion = acno_parts[1].lstrip('0')  # Remove the first two zeros
                record['acno'] = middle_portion
